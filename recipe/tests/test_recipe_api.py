...
class PrivateRecipeApiTest(TestCase):
    ...
    def test_partial_update_recipe(self):
        ...
        payload = dict(title="Chicken tikka", tags=[new_tag.id])
        url = details_url(recipe.id)
    ...
    def test_full_update_recipe(self):
        ...
        recipe = sample_recipe(user=self.user)
        recipe.tags.add(sample_tag(user=self.user))
        payload = dict(title="Spaghetti carbonara", time_minutes=25, price=5.00)
        url = details_url(recipe.id)
...
class RecipeImageUploadTests(TestCase):
    ...
    def test_upload_image_to_recipe(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".jpg") as ntf:
            ...
            ntf.seek(0)
            res = self.client.post(url, dict(image=ntf), format="multipart")
    ...
    def test_upload_image_bad_request(self):
        ...
        url = image_upload_url(self.recipe.id)
        res = self.client.post(url, dict(image="notimage"), format="multipart")
    ...
    def test_filter_recipes_by_tags(self):
        ...
        res = self.client.get(RECIPE_URL, dict(tags=f"{tag1.id}, {tag2.id}"))
    ...
    def test_filter_recipes_by_ingredients(self):
        ...
        res = self.client.get(
            RECIPE_URL, dict(ingredients=f"{ingredient1.id}, {ingredient2.id}")
        )
